...
// SPEED FIX: Switched to 'flux-1-schnell' for faster editing (Updated string)
...
async function streamToBase64(stream) {
    ...
    for (let i = 0; i < len; i += chunkSize) {
        binary += String.fromCharCode.apply(null, result.subarray(i, Math.min(i + chunkSize, len)));
    }
    return btoa(binary);
}
...
export async function onRequest(context) {
  ...
  try {
    ...
    // Expanded Edit Triggers for better detection
    const IMAGE_EDIT_TRIGGERS = [
        ...
        "transform", "again", "more", "different"
    ];

    if (mode === "chat") {
        if (IMAGE_GEN_TRIGGERS.some(t => cleanPrompt.includes(t))) {
           mode = "image_gen";
        } 
        // STRICT FIX: Check triggers regardless of 'chat' mode
        else if (IMAGE_EDIT_TRIGGERS.some(t => cleanPrompt.includes(t))) {
           // Check for image existence in history OR current input
           const lastImageCheck = await getLastImage(env, imgMemKey);
           
           // IF we have an image (either new upload or history), AUTO-SWITCH to edit.
           if (base64ImageInput || lastImageCheck) {
               mode = "image_edit";
           }
        }
    }
    ...
    if (
      ...
      const msg = success ? "Memory & Image History wiped successfully 🧹" : "No KV found or empty.";
    ...
    //////////////////////
    // IMAGE EDITING (FLUX 1 SCHNELL) - IMPROVED CHAINING
    //////////////////////
    if (mode === "image_edit") {
            // FIX: If no image provided in payload, try to fetch last image from KV (Chaining Support)
            let sourceBase64 = base64ImageInput;
            
            if (!sourceBase64) {
                sourceBase64 = await getLastImage(env, imgMemKey);
            }

            if (!sourceBase64) return new Response("No image provided for editing. Please upload or generate one first.", { status: 400, headers: cors });

            const imageBytes = base64ToUint8Array(sourceBase64);
            // FIX: Explicitly set MIME type for Flux
            ...
            form.append('input_image', imageBlob); // Fallback key
            
            // Adjust parameters for Flux Schnell/Dev
            form.append('strength', '0.6'); // 0.6 preserves structure while allowing changes
            ...
            try {
                ...
                if (fluxResponse instanceof ReadableStream) {
                    // CRITICAL FIX: TEE THE STREAM TO SAVE HISTORY FOR CHAINING
    ...
    //////////////////////
    // IMAGE GENERATION (LUCID ORIGIN) - IMPROVED CHAINING
    //////////////////////
    if (mode === "image_gen") {
      let width = 1024;
      let height = 1024;

 ...
 * UPDATE: RESTORED FLUX FOR IMAGE EDITING (REMOVED SD 1.5)
 * Author: M4 Spider
 * =========================================================
 */
...
// NOTE: Image Edit restored to Flux Schnell as per user request
const MODEL_IMAGE_EDIT = "@cf/black-forest-labs/flux-1-schnell";
...
export async function onRequest(context) {
  ...
  try {
    ...
    //////////////////////
    // IMAGE EDITING (FLUX RESTORED)
    //////////////////////
    // RESTORED: Uses Flux Schnell for editing as requested.
    // NOTE: This replaces the Stable Diffusion logic.
    if (mode === "image_edit") {
        ...
        // 3. Model Configuration (FLUX SCHNELL)
        // Reverting to Flux as per user request (no SD)
        const inputArgs = {
            prompt: editPrompt,
            image: [...imageArray], // Cloudflare AI expects array of numbers
            num_steps: 4, // Schnell defaults
            guidance: 0.0
        };

        try {
            const response = await runAi(env, MODEL_IMAGE_EDIT, inputArgs);

            // 5. Response Handling
            // Case A: Response is a direct stream
            ...
            // Case B: Response is a JSON object containing base64

// ai-worker.js - Cloudflare Worker for Spider AI (Option C: frontend + KV memory)
// Required bindings in the Worker environment (wrangler.toml):
// bindings:
//   - name: SPY_AI        (AI binding)
//   - name: CHAT_KV       (KV namespace for persisting chat history)
//   - name: USAGE_KV      (KV namespace for usage counters / quotas)
//   - name: IMAGE_KV      (KV namespace for stored image metadata/base64 if desired)
//
// Endpoints:
// POST /api/chat        -> { userId, message, history(optional), modePreference(optional:'mistral'|'ultra') }
// POST /api/search      -> { userId, query }
// POST /api/image       -> { userId, prompt }
// POST /api/file        -> { userId, filename, file_content }
// GET  /api/image/:id   -> returns base64 image metadata (json)
// POST /api/memory/clear -> { userId }  // optional to clear persistent memory
...
async function handleChat(request, env) {
  ...
  // Merge: frontendHistory may include latest messages; use it as freshest
  // Build messages array: system + merged history + new user message
  // We'll maintain persistentHistory as the primary source of truth, then append new frontend entries if provided
  ...
  if (modePref === "ultra") {
    ...
    if (u.allowed) {
      ...
      // fallback to default and tell user in payload (not in system prompt)
  ...
  // Prepare messages to send (system + trimmed merged)
  const messagesToAI = [
    { role: "system", content: SPIDER_SYSTEM_PROMPT },
    ...trimHistoryForModel(merged)
  ];

  // Try main model, if fails, fallback to other
  let aiResult;
  try {
  ...
  // Append assistant reply to merged history and persist (trim)
...
async function handleSearch(request, env) {
  ...
  // Use the search model binding via AI.run
  let searchResult;
  try {
  ...
  // Cloudflare search may return results array different shapes; try to extract
  const results = searchResult?.results || searchResult || {};

  // Summarize using LLM (mistral) with the Spider system prompt to keep tone
  let summaryResp;
  try {
...
async function handleImageGen(request, env) {
  ...
  // 1) Use LLM to enhance prompt (mistral)
...
`;
  ...
  // 2) Call SDXL base to generate image (attempt expects base64 or binary in response)
  let baseResp;
  try {
  ...
  // Extract base64 PNG if available
  let base64Image = extractImageBase64(baseResp);
  // If SDXL returns an object or textual base64, handle appropriately
  if (!base64Image) {
    // try refiner with textual output path or result as fallback
    // if no base64, attempt to parse textual result
    base64Image = extractText(baseResp) || null;
  }

  // 3) Optionally call refiner if we have base64
  ...
  if (base64Image) {
    try {
      ...
      // okay if refiner fails, continue with base
      console.warn("refiner failed:", e);
    }
  }

  // Ensure final returned image is base64 (strip data: if needed)
  if (refinedBase64 && refinedBase64.startsWith("data:")) {
    refinedBase64 = refinedBase64.split(",")[1];
  }

  // 4) Persist image metadata + prompt in IMAGE_KV
  ...
  try {
    // store metadata (and optionally the base64; be mindful of KV size - if huge, consider R2)
  ...
  // 5) Save a trace in user's persistent chat history (brief entry)
  try {
    const kvKey = HISTORY_KV_PREFIX + userId;
    const raw = await env.CHAT_KV.get(kvKey);
...
function mergeHistories(persistent, frontend) {
  // If frontend empty, just return persistent
  if (!frontend || frontend.length === 0) return persistent || [];
  // If persistent is empty, return frontend
  if (!persistent || persistent.length === 0) return frontend;
  // Heuristic: if frontend contains the last message in persistent, take frontend as latest
  ...
  if (lastPersistent && frontend[0] && lastPersistent.content === frontend[0].content) {
    // frontend likely contains continuation
    return [...persistent, ...frontend.slice(1)];
  }
  // otherwise, combine and dedupe by time/order - simplest: append frontend to persistent
...
function trimHistoryForModel(history) {
  ...
  // convert to messages as expected by env.AI.run (role + content)
...
function trimHistoryForStorage(history) {
  const start = Math.max(0, history.length - MAX_MESSAGES_TO_KEEP);
  const sliced = history.slice(start);
  // keep size in check by truncating long assistant or user messages
  return sliced.map(m => {
...
async function checkAndUpdateUltraUsage(env, userId) {
  ...
  try {
    ...
    // on error, be conservative and deny ultra to avoid abuse
...
function extractImageBase64(resp) {
  try {
    ...
    if (!t) return null;
    // some models return direct base64, sometimes with data: prefix
    const maybe = t.trim();
    if (maybe.startsWith("data:image/")) {
      const parts = maybe.split(",");
      return parts[1];
    }
    // if it's pure base64 PNG (very long) we can try to detect
    // a simplified check: if the string is long and contains '=', '/', '+'

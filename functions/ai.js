...
// =================== UPDATED EXTRACTOR (OLD STYLE + NEW FORMAT SUPPORT) =======================

function extractText(resp) {
  try {
    if (!resp) return "";

    // ========== YOUR OLD LOGIC (kept exactly same) ==========
    ...
    // NEW fallback: response_text
    if (resp.response_text) return String(resp.response_text).trim();

    // LAST RESORT: search JSON for any "text":"..."

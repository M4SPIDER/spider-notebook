// functions/ai.js
// Single Cloudflare Pages Function that ONLY handles /api/* routes and lets the main site load normally.
// Drop this into functions/ai.js and keep your wrangler.toml bindings (SPY_AI, CHAT_KV, USAGE_KV, IMAGE_KV).
...
// =================== ENTRYPOINT (Pages handler) =====================
export async function onRequest(context) {
  const { request } = context;
  const url = new URL(request.url);

  // Only handle /api/* paths here. Let the rest of the site run normally.
  if (!url.pathname.startsWith("/api/")) {
    // let Pages continue to serve static site
    return context.next();
  }

  // Route API request to our router
...
async function router(context) {
  ...
  try {
    ...
    return jsonResponse({ error: String(err || "internal_error") }, 500);
  }
}
...
async function handleChat(request, env) {
  ...
  // Merge histories (prefer frontend recent entries)
  ...
  const messagesToAI = [{ role: "system", content: SPIDER_SYSTEM_PROMPT }, ...trimHistoryForModel(merged)];
  ...
  const assistantText = extractText(aiResult).trim() || "No response from Spider.";
  ...
  try {
    ...
    console.warn("CHAT_KV.put failed:", e);
  }
...
async function handleSearch(request, env) {
  ...
  // Summarize results using LLM (mistral) to keep Spider tone
  let summaryResp;
  try {
...
`;
  ...
  try {
    ...
    console.warn("enhancer failed:", err);
    return jsonResponse({ error: "enhancer_failed" }, 502);
  }

  const enhanced = extractText(enhancerResp) || `${prompt}, cinematic lighting, full color, high detail`;
  ...
  if (!base64) base64 = extractText(baseResp) || null;

  // Refiner (optional)
  let refined = base64;
  if (base64) {
    try {
      const ref = await env.SPY_AI.run(SDXL_REFINER, { prompt: enhanced, image: base64, strength: 0.7 });
      const maybe = extractImageBase64(ref) || extractText(ref);
      if (maybe) refined = maybe;
    } catch (e) {
  ...
  // Persist metadata in IMAGE_KV (consider R2 if you store large blobs)
  const imageId = `img_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  const meta = { id: imageId, prompt: enhanced, userId, createdAt: Date.now(), base64: refined || null };
  try {
    await env.IMAGE_KV.put(IMAGE_KV_PREFIX + imageId, JSON.stringify(meta), { expirationTtl: 60 * 60 * 24 * 30 });
  } catch (e) {
    console.warn("IMAGE_KV.put failed:", e);
  }

  // Add trace to chat history
  try {
    const kvKey = HISTORY_KV_PREFIX + userId;
    const raw = await env.CHAT_KV.get(kvKey);
    let persistentHistory = raw ? JSON.parse(raw) : [];
    persistentHistory.push({ role: "assistant", content: `[image_created:${imageId}] ${meta.prompt}` });
  ...
  return jsonResponse({ imageId, prompt: meta.prompt, base64: meta.base64 });
}
